...
\begin{abstract}
This introduction to the \textsf{R} package \texttt{strucchange} is a (slighlty)
...
\begin{figure}[htbp]
\begin{center}
<<data,fig=TRUE,height=4,width=6,echo=FALSE>>=
library(strucchange)
data(USIncExp)
...
$x_{t-1}$ will be included as regressors.\\
